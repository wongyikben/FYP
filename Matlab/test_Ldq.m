...
% for i=1:72
%    if i == 1
%       x(1:3,i)=shit_solver(input(1:6,i),[2156 2367 2687]);
%       continue 
%    end
...
%(3,1:length(theta)) = (v*La.*1i*freq)./(r+Lb.*1i*freq+La.*1i*freq);
x(2,1:length(theta)) = (v*Lc.*1i*freq)./(r+Lb.*1i*freq+Lc.*1i*freq);
x(3,1:length(theta)) = (v*La.*1i*freq)./(r+Lc.*1i*freq+La.*1i*freq);
